#!/usr/bin/env python3
"""
🔧 Corregir opciones que revelan la respuesta (Versión final)

Corrige TODAS las opciones (correctas e incorrectas) que contengan
el nombre del país del enunciado para evitar revelar la respuesta.
"""
...
def fix_revealing_options():
    ...
    for file_path in sorted(question_files):
        ...
        for q in questions:
            ...
            # Verificar TODAS las opciones que contengan nombres de países
            new_options = []
            needs_fix = False
            
            for opt in options:
                if contains_country_name(opt, country_names):
                    # Generar alternativa
                    all_options = set(options)
                    available = all_correct_by_type[qtype] - all_options
                    
                    if available:
                        # Filtrar alternativas que no contengan nombres de países
                        valid_alternatives = []
