/// Script para generar preguntas de geografía desde REST Countries API
///
/// Uso:
/// dart scripts/generate_questions.dart
///
/// Este script genera preguntas de los 7 tipos:
/// 1. Siluetas (necesita imágenes)
/// 2. Banderas
/// 3. Capitales
/// 4. Población
/// 5. Ríos (necesita datos manuales)
/// 6. Fotos de ciudades (necesita imágenes)
/// 7. Extensión
...
void main() async {
  print('🌍 Generando preguntas para GeoQuiz Battle...\n');
  ...
  try {
    ...
    // Filtrar países con datos necesarios
    ...
    questions.addAll(_generateFlagQuestions(validCountries, ref: questionId));
    ...
    final outputFile = File('scripts/questions.json');
...
List<Map<String, dynamic>> _generateFlagQuestions(List countries, {required int ref}) {
  ...
  for (final country in countries) {
    ...
    if (usedCountries.length >= 100) break; // Máximo 100 preguntas de banderas
...
List<Map<String, dynamic>> _generateCapitalQuestions(List countries, {required int ref}) {
  ...
  for (final country in countries) {
    if (usedCountries.contains(country['cca2'])) continue;
    if (usedCountries.length >= 100) break; // Máximo 100 preguntas de capitales
...
List<Map<String, dynamic>> _generatePopulationQuestions(List countries, {required int ref}) {
  ...
  for (int i = 0; i < 50; i++) {
    // Seleccionar dos países aleatorios
    ...
    // Determinar cuál tiene mayor población
...
/// Generar opciones incorrectas para preguntas de países
...
/// Generar opciones incorrectas para preguntas de capitales
