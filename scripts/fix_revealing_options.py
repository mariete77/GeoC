#!/usr/bin/env python3
"""
🔧 Corregir opciones que revelan la respuesta (Versión inteligente)

Solo corregir cuando la opción CONTIENE el nombre del país del enunciado
en preguntas de moneda, idioma o capital.
"""
...
def extract_country_from_question(question: str) -> Set[str]:
    ...
    # Patrones comunes
    patterns = [
        r'de\s+([a-záéíóúñ\s]+)\??',  # "de Brasil"
        r'en\s+([a-záéíóúñ\s]+)\??',  # "en México"
        r'del\s+([a-záéíóúñ\s]+)\??',  # "del Perú"
        r'de\s+([a-záéíóúñ\s]+)\s+[a-záéíóúñ]+\??',  # "de Corea del Sur"
    ]
...
def fix_revealing_options():
    ...
    all_correct_by_type: Dict[str, Set[str]] = {
        'currency': set(),
        'language': set(),
        'capital': set(),
    }
    ...
    for file_path in sorted(question_files):
        ...
        for q in questions:
            ...
            # Extraer nombres de países del enunciado
            ...
            for opt in options:
                if opt != correct and contains_country_name(opt, country_names):
                    ...
                    if available:
                        # Filtrar alternativas que no contengan nombres del enunciado
                        valid_alternatives = []
